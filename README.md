...
* `version` (optional): controls the source version of objects used. This only applies for `d3fend-attack` b/c the way relationship generation happens internally (that is all data is already held in the knowledgebase). So if you are using D3FEND `1.3.0` you should pass that here, to ensure relationships to other versions are not created. This will not affect the `_is_latest` behaviour, b/c this always considers the highest modified time, regardless of version passed.
